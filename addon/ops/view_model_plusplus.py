...
class SOURCEOPS_OT_ViewModelPlusPlus(bpy.types.Operator):
    bl_idname = 'sourceops.view_model_plusplus'
    bl_options = {'REGISTER'}
    bl_label = 'View Model in HLMV++'
    ...
    def invoke(self, context, event):
        ...
        error = source_model.view_model_plusplus()

        if error:
            self.report({'ERROR'}, error)
            return {'CANCELLED'}

        self.report({'INFO'}, f'Viewing model in HLMV++')
        return {'FINISHED'}